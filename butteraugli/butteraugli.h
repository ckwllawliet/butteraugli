...
namespace butteraugli {
    ...
    bool ButteraugliAdaptiveQuantization(size_t xsize, size_t ysize, const std::vector<std::vector<float> > &rgb, std::vector<float> &quant);
...
#define BUTTERAUGLI_ASSUME_ALIGNED(ptr, align) (ptr)
    ...
    public:
        ...
        Image(const size_t xsize, const size_t ysize, uint8_t * const BUTTERAUGLI_RESTRICT bytes, const size_t bytes_per_row)
            : xsize_(xsize),
            ysize_(ysize),
        ...
        const uint8_t * const BUTTERAUGLI_RESTRICT bytes() const { return bytes_.get(); }
        ...
        intptr_t PixelsPerRow() const {
            static_assert(CacheAligned::kCacheLineSize % sizeof(T) == 0, "Padding must be divisible by the pixel size.");
    ...
    template <typename T>
    static inline std::vector<Image<T>> CreatePlanes(const size_t xsize, const size_t ysize, const size_t num_planes) {
    ...
    template <typename T>
    static inline std::vector<Image<T>> CopyPlanes(const std::vector<Image<T>> &planes) {
    ...
    static inline void CopyFromPacked(const std::vector<T> &from, Image<T> *to) {
        ...
        for (size_t y = 0; y < ysize; ++y) {
            const float* const BUTTERAUGLI_RESTRICT row_from = from.data() + y * xsize;
    ...
    template <typename T>
    static inline std::vector<Image<T>> PlanesFromPacked(const size_t xsize, const size_t ysize, const std::vector<std::vector<T>> &packed) {
        std::vector<Image<T>> planes;
    ...
    template <typename T>
    static inline std::vector<std::vector<T>> PackedFromPlanes(const std::vector<Image<T>> &planes) {
        assert(!planes.empty());
    ...
    public:
        ...
        void DiffmapOpsinDynamicsImage(const std::vector<ImageF>& xyb1, ImageF& result) const;
        ...
        void Mask(std::vector<ImageF>* BUTTERAUGLI_RESTRICT mask, std::vector<ImageF>* BUTTERAUGLI_RESTRICT mask_dc) const;
    ...
    void ButteraugliDiffmap(const std::vector<ImageF> &rgb0, const std::vector<ImageF> &rgb1, ImageF &diffmap);
    ...
    template <int INDEX>
    static inline void ClenshawRecursion(const double x, const double *coefficients, double *b1, double *b2) {
    ...
    // Base case
    template <>
    inline void ClenshawRecursion<0>(const double x, const double *coefficients, double *b1, double *b2) {
        const double x_b1 = x * (*b1);
    ...
    struct RationalPolynomial {
        template <int N>
        static double EvaluatePolynomial(const double x, const double(&coefficients)[N]) {
            double b1 = 0.0;
            double b2 = 0.0;
