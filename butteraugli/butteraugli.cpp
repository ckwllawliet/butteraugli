...
//     x is roughly red-subtract-green.     r-g
//     y is yellow.                         r+g
//     b is blue.                           b
...
#define CHECK_IMAGE(image, name)
    ...
    //scaler = -1/(2*sigma^2), diff = max(1, m*|sigma|), kernel = exp[-(i^2)/(2*sigma^2)], Gaussian Kernel
    ...
    //convolve the left & right border column with kernel
    void ConvolveBorderColumn(const ImageF& in, const std::vector<float>& kernel, const float weight_no_border, const float border_ratio,
        const size_t x, float* const BUTTERAUGLI_RESTRICT row_out) {
        const int offset = kernel.size() / 2;       //diff
        int minx = x < offset ? 0 : x - offset;
        int maxx = std::min<int>(in.xsize() - 1, x + offset);
        float weight = 0.0f;
        for (int j = minx; j <= maxx; ++j) {
            weight += kernel[j - x + offset];       //kernel[0] ~ kernel[2*offset]
        }
        // Interpolate linearly between the no-border scaling and border scaling.
        weight = (1.0f - border_ratio) * weight + border_ratio * weight_no_border;      //(1 - r) * w + r * w_n
    ...
    ImageF Convolution(const ImageF& in, const std::vector<float>& kernel, const float border_ratio) {
        ...
        // left border
        for (; x < border1; ++x) {
            ConvolveBorderColumn(in, kernel, weight_no_border, border_ratio, x, out.Row(x));
        }
        // middle
        for (; x < border2; ++x) {
    ...
    ImageF Blur(const ImageF& in, float sigma, float border_ratio) {
        ...
        return Convolution(Convolution(in, kernel, border_ratio), kernel, border_ratio);
    }
    ...
    // Difference = 1.5 * blur(sigma) - 0.5 * blur(sigma * 2)
    ...
    // res = array[base] + mix * (array[base+1] - arrasy[base])
    inline double InterpolateClampNegative(const double *array, int size, double ix) {
        if (ix < 0) {
            ix = 0;
        }
    ...
    double GammaMinArg() {
        ...
        OpsinAbsorbance(0.0, 0.0, 0.0, &out0, &out1, &out2);            //opsin absorbance matrix
        return std::min(out0, std::min(out1, out2));
    }

    double GammaMaxArg() {
        double out0, out1, out2;
        OpsinAbsorbance(255.0, 255.0, 255.0, &out0, &out1, &out2);      //opsin absorbance matrix
    ...
    // yb_blur = yb_blur * (c0.y+c1.y) / (yy_blur + eps)
    ImageF BlurredBlueCorrelation(const std::vector<ImageF>& uhf, const std::vector<ImageF>& hf) {
        ...
        for (size_t y = 0; y < ysize; ++y) {
            ...
            for (size_t x = 0; x < xsize; ++x) {
                ...
                row_yb[x] = yval * bval;                                //yb=(c0.y+c1.y)*(c0.b+c1.b)
                row_yy[x] = yval * yval;                                //yy=(c0.y+c1.y)*(c0.y+c1.y)
            }
        }
        ...
        for (size_t y = 0; y < ysize; ++y) {
            ...
            for (size_t x = 0; x < xsize; ++x) {
                ...
                const float yval = row_hf_y[x] + row_uhf_y[x];
                row_yb[x] *= yval / (row_yy[x] + epsilon);              //yb_blur = yb_blur * (c0.y+c1.y) / (yy_blur + eps) 
            }
        }
        return yb_blurred;
    }

    //retval = svale * {offset + pow[v-...(v-limit)*mul..., kGamma]}
    ...
    //sensitivity = Gamma[Opsin(blurred)] / [Opsin(blurred)]
    //xyb = RgbToXyb[sensitivity * Opsin(rgb)]
        ...
        for (size_t y = 0; y < rgb[0].ysize(); ++y) {
            ...
            for (size_t x = 0; x < rgb[0].xsize(); ++x) {
                float sensitivity[3];
                {
                    ...
                    OpsinAbsorbance(row_blurred_r[x], row_blurred_g[x], row_blurred_b[x], &pre_mixed0, &pre_mixed1, &pre_mixed2);
                ...
                OpsinAbsorbance(row_r[x], row_g[x], row_b[x], &cur_mixed0, &cur_mixed1, &cur_mixed2);
                cur_mixed0 *= sensitivity[0];
                cur_mixed1 *= sensitivity[1];
                cur_mixed2 *= sensitivity[2];
                //xyb = RgbToXyb[sensitivity * Opsin(rgb)]
                RgbToXyb(cur_mixed0, cur_mixed1, cur_mixed2, &row_out_x[x], &row_out_y[x], &row_out_b[x]);
            }
        }
        return xyb;
    }
    ...
    //x < |w| -> 2*x; x > w -> x+w; x < w -> x-w
    static BUTTERAUGLI_INLINE float AmplifyRangeAroundZero(float w, float x) {
        return x > w ? x + w : x < -w ? x - w : 2.0f * x;
    }

    //copy in[2]£º remove when warray[0/1] >= 0; amplify when warray[0/1] < 0
    std::vector<ImageF> ModifyRangeAroundZero(const double warray[2], const std::vector<ImageF>& in) {
            ...
            for (int y = 0; y < plane.ysize(); ++y) {
                ...
                if (k == 2) {
                    ...
                    for (int x = 0; x < plane.xsize(); ++x) {
                        row_out[x] = RemoveRangeAroundZero(w, row_in[x]);           //remove when warray[0/1] >= 0
                    }
                }
                else {
                    const double w = -warray[k];
                    for (int x = 0; x < plane.xsize(); ++x) {
                        row_out[x] = AmplifyRangeAroundZero(w, row_in[x]);          //amplify when warray[0/1] < 0
                    }
                }
            }
    ...
    // new = hf * mul * reg / (reg + brightness)
    static ImageF SuppressHfInBrightAreas(size_t xsize, size_t ysize, const ImageF& hf, const ImageF& brightness) {
    ...
    // new = (ix - yw) * 0.7 + yw; new = (ix + yw) * 0.7 - yw
    static ImageF MaximumClamping(size_t xsize, size_t ysize, const ImageF& ix, double yw) {
        ImageF inew(xsize, ysize);
        for (size_t y = 0; y < ysize; ++y) {
    ...
    // suppress ix by w * iy
    static ImageF SuppressXByY(size_t xsize, size_t ysize, const ImageF& ix, const ImageF& iy, const double w) {
        ImageF inew(xsize, ysize);
        for (size_t y = 0; y < ysize; ++y) {
            const float* const rowx = ix.Row(y);
    ...
    // separate frequencies lf/mf/hf/uhf from xyb
    static void SeparateFrequencies(size_t xsize, size_t ysize, const std::vector<ImageF>& xyb, PsychoImage &ps) {
        PROFILER_FUNC;
        ps.lf.resize(3);
        ps.mf.resize(3);
        ps.hf.resize(3);
        ps.uhf.resize(3);
        //extract lf/mf/hf/uhf from xyb
        for (int i = 0; i < 3; ++i) {
            // Extract lf ...
            ...
            for (size_t y = 0; y < ysize; ++y) {
                for (size_t x = 0; x < xsize; ++x) {
                    ps.mf[i].Row(y)[x] = xyb[i].Row(y)[x] - ps.lf[i].Row(y)[x];       //mf = xyb -lf
                }
            }
    ...
    // row_diff_x = w * diff[blurred(row0), blurred(row1)]^2; row[x] -= row[x-1]
    static void SameNoiseLevelsX(const ImageF& i0, const ImageF& i1, const double kSigma, const double w, const double maxclamp, ImageF* BUTTERAUGLI_RESTRICT diffmap) {
    ...
    // row_diff_y = w * diff[blurred(row0), blurred(row1)]^2; row[y] -= row[y-1]
    static void SameNoiseLevelsY(const ImageF& i0, const ImageF& i1, const double kSigma, const double w, const double maxclamp, ImageF* BUTTERAUGLI_RESTRICT diffmap) {
        ImageF blurred0 = CopyPixels(i0);
        ImageF blurred1 = CopyPixels(i1);
    ...
    // row_diff_y = w * diff[blurred(row0), blurred(row1)]^2; row[y] -= row[y-1]; row[0] = 0.25 * row[1]
    static void SameNoiseLevelsYP1(const ImageF& i0, const ImageF& i1, const double kSigma, const double w, const double maxclamp, ImageF* BUTTERAUGLI_RESTRICT diffmap) {
        ImageF blurred0 = CopyPixels(i0);
        ImageF blurred1 = CopyPixels(i1);
        for (size_t y = i0.ysize() - 1; y != 0; --y) {
            float* BUTTERAUGLI_RESTRICT const row0prev = blurred0.Row(y - 1);
    ...
    // row_diff_y = w * diff[blurred(row0), blurred(row1)]^2; row[y] -= row[y-1]; row[size-1] = 0.25 * row[size-2]
    static void SameNoiseLevelsYM1(const ImageF& i0, const ImageF& i1, const double kSigma, const double w, const double maxclamp, ImageF* BUTTERAUGLI_RESTRICT diffmap) {
        ImageF blurred0 = CopyPixels(i0);
        ImageF blurred1 = CopyPixels(i1);
        for (size_t y = i0.ysize() - 1; y != 0; --y) {
            float* BUTTERAUGLI_RESTRICT const row0prev = blurred0.Row(y - 1);
    ...
    // row_diff = w * diff^2
    static void L2Diff(const ImageF& i0, const ImageF& i1, const double w, ImageF* BUTTERAUGLI_RESTRICT diffmap) {
        for (size_t y = 0; y < i0.ysize(); ++y) {
    ...
    // n == 1: row_diff = w * |diff|; n == 2: row_diff = w * diff^2; n == 3: row_diff = w * pow(diff,n)
    static void LNDiff(const ImageF& i0, const ImageF& i1, const double w, double n, ImageF* BUTTERAUGLI_RESTRICT diffmap) {
        if (n == 1.0) {
            for (size_t y = 0; y < i0.ysize(); ++y) {
                const float* BUTTERAUGLI_RESTRICT const row0 = i0.Row(y);
                const float* BUTTERAUGLI_RESTRICT const row1 = i1.Row(y);
    ...
    // diffmap = scale * [mul1 + blur(orig_val^2)]
    ImageF CalculateDiffmap(const ImageF& diffmap_in) {
        ...
        for (size_t y = 0; y < diffmap.ysize(); ++y) {
            ...
            for (size_t x = 0; x < diffmap.xsize(); ++x) {
                ...
                // TODO(b/29974893): Until that is fixed do not call sqrt on very small numbers.
                row_out[x] = (orig_val < (1.0f / (kInitialSlope * kInitialSlope)) ? kInitialSlope * orig_val : std::sqrt(orig_val));
            }
        }
        {
    ...
    // row = a * uhf + b * hf; mask(xyb0, xyb1)
    void MaskPsychoImage(const PsychoImage& pi0, const PsychoImage& pi1, const size_t xsize, const size_t ysize,
        std::vector<ImageF>* BUTTERAUGLI_RESTRICT mask, std::vector<ImageF>* BUTTERAUGLI_RESTRICT mask_dc) {
        std::vector<ImageF> mask_xyb0 = CreatePlanes<float>(xsize, ysize, 3);
        std::vector<ImageF> mask_xyb1 = CreatePlanes<float>(xsize, ysize, 3);
        static const double muls[4] = {0, 1.75262681671, 0.962073813832, 2.587167299, };
        for (int i = 0; i < 2; ++i) {
            double a = muls[2 * i];
            double b = muls[2 * i + 1];
            for (size_t y = 0; y < ysize; ++y) {
    ...
    // xyb = OpsinDynamicsImage(rgb); SeperateFrequencies(xyb)
    ...
    void ButteraugliComparator::Mask(std::vector<ImageF>* BUTTERAUGLI_RESTRICT mask, std::vector<ImageF>* BUTTERAUGLI_RESTRICT mask_dc) const {
        MaskPsychoImage(pi0_, pi0_, xsize_, ysize_, mask, mask_dc);
    }

    void ButteraugliComparator::Diffmap(const std::vector<ImageF>& rgb1, ImageF &result) const {
        PROFILER_FUNC;
        if (xsize_ < 8 || ysize_ < 8) return;
        DiffmapOpsinDynamicsImage(OpsinDynamicsImage(rgb1), result);
    }

    void ButteraugliComparator::DiffmapOpsinDynamicsImage(const std::vector<ImageF>& xyb1, ImageF &result) const {
        PROFILER_FUNC;
        if (xsize_ < 8 || ysize_ < 8) return;
        PsychoImage pi1;
    ...
    // MaltaDiffMap -> SameNoiseLevels -> LNDiff -> L2Diff -> MaskPsychoImage -> CalculateDiffMap
    void ButteraugliComparator::DiffmapPsychoImage(const PsychoImage& pi1, ImageF& result) const {
        PROFILER_FUNC;
        ...
        MaltaDiffMap(pi0_.uhf[1], pi1.uhf[1], wUhfMalta, norm1Uhf, &block_diff_ac[1]);

        static const double wUhfMaltaX = 3.36199686627;
        static const double norm1UhfX = norm1Uhf;
        MaltaDiffMap(pi0_.uhf[0], pi1.uhf[0], wUhfMaltaX, norm1UhfX, &block_diff_ac[0]);

        static const double wHfMalta = 15.6469934822;
        static const double norm1Hf = norm1Uhf;
        MaltaDiffMap(pi0_.hf[1], pi1.hf[1], wHfMalta, norm1Hf, &block_diff_ac[1]);

        static const double wHfMaltaX = 129.122071602;
        static const double norm1HfX = norm1Uhf;
        MaltaDiffMap(pi0_.hf[0], pi1.hf[0], wHfMaltaX, norm1HfX, &block_diff_ac[0]);

        static const double wMfMaltaX = 51.2720081112;
        static const double norm1MfX = norm1Uhf;
        MaltaDiffMap(pi0_.mf[0], pi1.mf[0], wMfMaltaX, norm1MfX, &block_diff_ac[0]);
        ...
        SameNoiseLevelsX(pi0_.hf[1], pi1.hf[1], kSigmaHfX, wmul[10], maxclamp, &block_diff_ac[1]);
        SameNoiseLevelsY(pi0_.hf[1], pi1.hf[1], kSigmaHfX, wmul[10], maxclamp, &block_diff_ac[1]);
        SameNoiseLevelsYP1(pi0_.hf[1], pi1.hf[1], kSigmaHfX, wmul[10], maxclamp, &block_diff_ac[1]);
        SameNoiseLevelsYM1(pi0_.hf[1], pi1.hf[1], kSigmaHfX, wmul[10], maxclamp, &block_diff_ac[1]);
        ...
        L2Diff(blurred_b_y_correlation0, blurred_b_y_correlation1, wBlueCorr, &block_diff_ac[2]);

        std::vector<ImageF> mask_xyb;
        std::vector<ImageF> mask_xyb_dc;
        MaskPsychoImage(pi0_, pi1, xsize_, ysize_, &mask_xyb, &mask_xyb_dc);

        result = CalculateDiffmap(CombineChannels(mask_xyb, mask_xyb_dc, block_diff_dc, block_diff_ac));
    }

    // Sum of different kinds of units
    static float MaltaUnit(const float *d, const int xs) {
        ...
        static const float kEdgemul = 0.0309255573587;
        {
            ...
            float sum2 =
                ...
                d[xs + 4];
            float edge = sum - sum2;
            retval += kEdgemul * edge * edge;           //sum^2 + k * (sum - sum2)^2
        }
        {
            // y grows, x constant
            float sum =
    ...
    void ButteraugliComparator::MaltaDiffMap(const ImageF& y0, const ImageF& y1, const double weight, const double norm1, ImageF* BUTTERAUGLI_RESTRICT block_diff_ac) const {
        PROFILER_FUNC;
        ...
        for (size_t y = 0, ix = 0; y < ysize_; ++y) {
            ...
            for (size_t x = 0; x < xsize_; ++x, ++ix) {
                ...
                diffs[ix] = scaler * diff;      // diffs = (row0 - row1) * norm2 / [norm1 + 0.5 * (|row0|+|row1|)] 
            }
        }
