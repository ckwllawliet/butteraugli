...
namespace butteraugli {
    namespace {
        ...
        //0 - 255 -> 255 * {i/255 <= 0.04045 ? i/255/12.92 : [(i/255 + 0.055)/1.055]^2.4}
            ...
            if (rgb.size() == 3) {  // RGB
                ...
                for (int c = 0; c < 3; c++) {
                    ...
                    for (int y = 0; y < ysize; ++y) {
                        ...
                        for (size_t x = 0; x < xsize; x++) {
                            int value;
                            //value = [rgb * alpha + back * (255-alpha) + 127] / 255
                            if (row_alpha[x] == 255) {
                                value = row_rgb[x];
                            }
        ...
        //Construct RGB according to the score 
        static void ScoreToRgb(double score, double good_threshold, double bad_threshold, uint8_t rgb[3]) {
            ...
            static const int kTableSize = sizeof(heatmap) / sizeof(heatmap[0]);   //12*3 / 3 = 12
            //score = min{max[score * (size-1), 0], size-2}
            score = std::min<double>(std::max<double>(score * (kTableSize - 1), 0.0), kTableSize - 2);
            int ix = static_cast<int>(score);       //ix = |score|
            double mix = score - ix;                //mix = score - |score| 
            for (int i = 0; i < 3; ++i) {
        ...
        //Create image according to the distmap & heatmap
        void CreateHeatMapImage(const ImageF& distmap, double good_threshold, double bad_threshold, size_t xsize, size_t ysize, std::vector<uint8_t>* heatmap) {
        ...
        int Run(int argc, char* argv[]) {
            ...
            if (rgb1[0].xsize() != rgb2[0].xsize() || rgb1[0].ysize() != rgb2[0].ysize()) {
                fprintf(
            ...
            double diff_value;
            if (!butteraugli::ButteraugliInterface(linear1, linear2, diff_map, diff_value)) {
                fprintf(stderr, "Butteraugli comparison failed\n");
                return 1;
            }
            ImageF* diff_map_ptr = &diff_map;
            //Overlay the image over s white background.
            if (rgb1.size() == 4 || rgb2.size() == 4) {
                ...
                if (!butteraugli::ButteraugliInterface(linear1, linear2, diff_map_on_white, diff_value_on_white)) {
                    fprintf(stderr, "Butteraugli comparison failed\n");
                    return 1;
                }
            ...
            if (argc == 4) {
                ...
                CreateHeatMapImage(*diff_map_ptr, good_quality, bad_quality, rgb1[0].xsize(), rgb2[0].ysize(), &rgb);
